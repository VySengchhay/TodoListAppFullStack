package com.androidapp.todolistapplication.feature.auth.fake
 ...
 *
 * - Set the `*Result` properties to choose what the "backend" answers.
 * - Read the `*Calls` lists to check what the code under test sent.
 * - Set [delayMillis] to keep calls in flight, so loading states can be asserted.
 * - Set [exceptionToThrow] to simulate a crash inside the repository.
 */
